...
export const OutputPanelContext = createContext(null)
export const OutputMessageContext = createContext(null)
...
export function OutputActions({ code, language, output: supplied, disabled = false }) {
  const onOpen = useContext(OutputPanelContext)
  const messageId = useContext(OutputMessageContext)
  ...
  return <span className="output-actions">
    <Button size="sm" variant="ghost" disabled={disabled || oversized} onClick={() => { if (!onOpen || onOpen(output, messageId) === false) { setSource(false); setOpen(true) } }}>Preview</Button>
    ...
    <Modal open={open} onClose={() => setOpen(false)} title="Output preview" labelledById="output-preview-title" size="xl" className="output-modal" footer={<Button variant="primary" onClick={download}>Download file</Button>}>
      ...
      <div className="output-preview">{source ? <pre>{output.text}</pre> : <OutputPreview output={output} />}</div>
    </Modal>
  </span>
}
