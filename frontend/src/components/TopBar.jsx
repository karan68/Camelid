...
function TopBar({
  ...
  const tone = gate.chatUnlocked || gate.embeddingOnly ? 'ready' : apiUnavailable ? 'offline' : runtime?.loaded_now ? 'warn' : 'neutral'
      ...
      {!demoMode && (
        <div className="topbar__gate">
          ...
          <button
            ...
            title={gate.chatUnlocked ? `${modelName} is ready` : gate.embeddingOnly ? `${modelName} is ready for embeddings, not Chat` : 'Open Models to load or switch models'}
          >
