...
export const LLAMA32_3B_ACCEPTANCE_SUMMARY = 'This exact 3B Q8_0 row is smoke-supported for local chat after exact-row /api/models/load, /v1/completions, /v1/chat/completions, frontend smoke, compact parity, broader three-prompt 50-token parity, a five-prompt API smoke pack, current-main API/WebUI support-gate refresh, and bounded 512/1024/2048-context parity packs. The frontend shows template/Jinja as row-scoped readiness from /api/capabilities, keeps bounded perf/RSS separate from production-throughput support, and production throughput remains unpromoted; it does not promote neighboring Llama sizes or model-native/larger contexts beyond the checked packs, and the 1B/8B wins remain their own exact rows, not inherited from this 3B row.'
