...
import { isEmbeddingOnlyModel, isGenerationCapableModel, modelCapabilityFields } from '../lib/modelCapabilities.js'
...
function uniqueLocalRecordByFilename(localRecords, filename) {
  if (!filename) return null
  const matches = localRecords.filter((record) => modelFilename(record) === filename)
...
export function mergeModelLists({ modelItems, health, currentModel, localModels, apiBase, localFacts }) {
  ...
  modelItems.forEach((item) => {
    const localRecord = localRecords.find((record) => localRecordMatchesBackendId(record, item.id))
      || uniqueLocalRecordByFilename(localRecords, modelFilename(item))
      || (health?.active_model_id === item.id && activeFilename
        ? uniqueLocalRecordByFilename(localRecords, activeFilename)
        : null)
      || null
