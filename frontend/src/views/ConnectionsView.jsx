...
export default function ConnectionsView({ mcp }) {
  ...
  const openForm = () => { setEditingId(null); setForm(EMPTY); setFormError(''); setShowForm(true) }
  const edit = config => {
    setEditingId(config.id)
    setForm({ ...EMPTY, ...config, args: JSON.stringify(config.args || []), env_vars: (config.env_vars || []).join(', ') })
    setFormError(''); setShowForm(true)
  }
  const test = async id => {
    setDiagnostics(current => ({ ...current, [id]: null }))
    const result = await mcp.mutate('/connections/' + id + '/test', { method: 'POST' })
    if (result) setDiagnostics(current => ({ ...current, [id]: { ...result, checkedAt: new Date().toLocaleTimeString() } }))
  }
  ...
  const save = async event => {
    ...
    const result = await mcp.mutate(editingId ? '/connections/' + editingId : '/connections', { method: editingId ? 'PUT' : 'POST', body: {
      ...form, name: form.name.trim(), args,
      env_vars: form.env_vars.split(',').map(name => name.trim()).filter(Boolean),
    } })
    if (result) { setExpanded(result.config.id); setForm(EMPTY); setShowForm(false); setFilter('all') }
  }
  ...
  return <section className="cxv mcp-view">
    ...
    {mcp.connections.length > 0 && <div className="mcp-connection-list">
      {visible.map(connection => {
        ...
        const diagnostic = diagnostics[config.id]
        return <article className="mcp-connection" key={config.id}>
          <div className="mcp-connection__row"><span className="mcp-connection__icon">{local ? <IconServer size={21} /> : <IconLink size={21} />}</span>
            ...
            <div className="mcp-connection__buttons">
              {connected && <Button size="sm" disabled={mcp.busy} icon={<IconRefresh size={14} />} onClick={() => test(config.id)}>Test connection</Button>}
              <Button size="sm" variant="ghost" disabled={mcp.busy} onClick={() => edit(config)}>Edit settings</Button>
          ...
          {diagnostic && <div className={'mcp-diagnostics' + (diagnostic.ok ? ' is-passed' : ' is-failed')} role="status">
            <strong>{diagnostic.ok ? 'Connection test passed' : 'Connection test failed'}</strong><span>{diagnostic.message}</span>
            {diagnostic.ok && <span>{diagnostic.tools_count} tools available · No tools executed</span>}
            <small>Checked {diagnostic.checkedAt} · {diagnostic.elapsed_ms} ms</small>
          </div>}
    ...
    <Modal open={showForm} onClose={closeForm} title={editingId ? "Edit connection settings" : "Connect a tool server"} labelledById="mcp-add-server-title" className="mcp-connection-modal">
      <form className="mcp-form" onSubmit={save}>
        ...
        <p className="mcp-muted">{editingId ? 'Saving disconnects this server and cancels its pending calls. Reconnect and select its tools again to use the updated settings.' : 'Saving does not start the server. Choose Connect when you’re ready.'}</p>
        {(formError || mcp.error) && <p role="alert" className="mcp-error">{formError || mcp.error}</p>}
        <div className="mcp-form__footer"><Button onClick={closeForm} variant="ghost" disabled={mcp.busy}>Cancel</Button><Button type="submit" variant="primary" loading={mcp.busy}>{editingId ? 'Save settings' : 'Save server'}</Button></div>
      </form>
    </Modal>
