...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: '8B 1024/2048 bounded-context promotion, model-native/larger context beyond the checked 512 pack, arbitrary templates, throughput, portability, and durable synchronized bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'blocked_backend_timeout_900s', bounded_context_2048_pack: 'blocked_backend_timeout_900s', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-512', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512-context pack, and compact template-shapes pack evidence; Camelid supports exact-row smoke plus that checked bounded pack only. The 1024/2048 buckets remain red/not promoted pending fresh PASS artifacts and synchronized docs/API/frontend alignment' },
...
assert.match(
  ...
  'ModelsView must keep 8B 1024/2048 red until fresh current-head PASS plus docs/API/frontend alignment',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 bounded packs passed for the exact tracked row only/,
  'ModelsView must not show stale green 8B 1024/2048 copy while API capabilities keep those buckets red',
)
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512-context pack, and compact template-shapes pack evidence; Camelid supports exact-row smoke plus that checked bounded pack only/)
