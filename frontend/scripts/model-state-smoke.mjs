...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_bounded_pack_support', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond the checked 512/1024/2048 bounded packs, arbitrary templates, throughput, portability, and durable normalized full-support bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'validated_second_pack', bounded_context_2048_pack: 'validated_third_pack', latest_checked_bucket: 'llama3-context-2048-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-204', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512/1024/2048 context packs, compact template-shapes pack evidence, and fresh 1024/2048 PASS artifacts from source head e203d3cf3ea5. Camelid supports exact-row smoke plus those checked bounded packs only. No model-native/larger context, broad/full support, neighboring row, arbitrary template, throughput, or portability claim is implied' },
...
assert.match(
  modelsViewSource,
  /pin-badge ready[^>]*>8B 1024\/2048 bounded packs passed for exact row</,
  'ModelsView must show the green 8B 1024/2048 bounded-pack badge after fresh PASS artifacts plus docs/API/frontend alignment',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 remain red pending fresh current-head PASS/,
  'ModelsView must not keep stale red 8B 1024/2048 copy after API capabilities and evidence promote the checked bounded packs',
)
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512\/1024\/2048 context packs, compact template-shapes pack evidence, and fresh 1024\/2048 PASS artifacts from source head e203d3cf3ea5/)
