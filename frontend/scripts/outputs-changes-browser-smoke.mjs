...
try {
  ...
  await page.waitForSelector('.conversation-files iframe')
  assert.equal(await page.$eval('.conversation-files iframe', e => e.getAttribute('sandbox')), '')
  ...
  await clickText('.conversation-files button','Download file')
  ...
  await page.click('.conversation-files button[aria-label="Close conversation files"]')
  for (const index of [1,2,3]) {
    const cards = await page.$$('.message-code-card')
    await (await cards[index].$('.output-actions button')).click()
    await page.waitForSelector('.conversation-files')
  ...
  await page.waitForSelector('[aria-label="Conversation files"]')
  await page.click('[aria-label="Conversation files"]')
  await page.waitForSelector('[aria-label="Choose conversation file"]')
  assert.equal(await page.$$eval('[aria-label="Choose conversation file"] option', items => items.length), 4, 'all files are recovered from the transcript')
  const firstFile = await page.$eval('[aria-label="Choose conversation file"] option', option => option.value)
  await page.select('[aria-label="Choose conversation file"]', firstFile)
  assert.equal(await page.$eval('[aria-label="Output filename"]', input => input.value), 'report.html', 'renamed filename survives reload')
  await clickText('.conversation-files button', 'Source')
  assert.equal(await page.$eval('.conversation-files pre', pre => pre.textContent), htmlSource)
  ...
  assert.equal(await page.$eval('.conversation-files', e => e.getAttribute('aria-modal')), 'true')
  await page.keyboard.press('Tab')
  assert.equal(await page.$eval('.conversation-files', e => e.contains(document.activeElement)), true)
  await page.keyboard.press('Escape')
  await page.waitForSelector('.conversation-files', {hidden:true})
