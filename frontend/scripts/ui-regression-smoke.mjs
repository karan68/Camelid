...
assert.match(chatWorkspaceSource, /showStreamingStatus && <StreamingStatus/, 'pre-token assistant rows should render the active status badge before streamed content')
...
assert.match(chatWorkspaceSource, /showStreamingStatus && <StreamingStatus[^\n]*tail/, 'pre-token streaming assistant rows should keep the active status after the placeholder too')
