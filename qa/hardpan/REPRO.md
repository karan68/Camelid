...
| **A12** | Phase 3 / W5 | The conductor's prescribed fix (a Ctrl+V arm in `on_key`) **cannot fire under default Windows hosts**: the Phase 0 paste log contains *no* `Char('v')+CONTROL` event — the terminal intercepts the chord itself and injects the clipboard as plain keystrokes, which is exactly the multi-goal bug. The Ctrl+V arm ships anyway (it covers hosts that pass the chord through: custom WT keybindings, SSH), but the fix is **extended** with a queued-input Enter guard: an Enter processed while more input is already queued (`event::poll(0)`) is a pasted newline, not a submit. Near-simultaneous arrival is not human typing (the run loop drains one event per iteration with ~sub-ms gaps). Known edge, accepted: an Enter as the *final* pasted character (clipboard ending in `\n`) still submits — which for a pasted goal is the desired outcome. Unix path unchanged (flag is constant false; bracketed paste continues to handle it). | 3 |
