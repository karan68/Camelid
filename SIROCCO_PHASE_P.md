...
- **M2b — enlarge Bq (the better remaining lever, unbuilt).** Raising the prefill chunk (decoupled from `MAX_VERIFY_K`=8, with a vlogits-free prefill scratch, GEMM chunk ≤32) cuts the **chunk count** so the growing prefix is re-read fewer times. Unlike GQA, this saving is **real DRAM** (chunks are temporally separated, so the prefix is evicted between them — not L2-absorbed). More promising than M2a; more integration work.
