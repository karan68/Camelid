...
for (const mutate of [
  (receipt) => { receipt.inspection.alignment = 24 },
  (receipt) => { receipt.inspection.data_start_offset = 95 },
  (receipt) => { receipt.inspection.alignment = 16; receipt.inspection.data_start_offset = 16 },
  (receipt) => { receipt.inspection.tensor_inventory.total_n_bytes = 5 },
  (receipt) => { receipt.inspection.tensor_inventory.total_n_bytes = 3 },
  (receipt) => { delete receipt.inspection.tensor_inventory.types },
  (receipt) => { receipt.inspection.tensor_inventory.types = { F32: 1, Q8_0: 3 } },
  (receipt) => {
  ...
  assert.equal(rejected.error_code, 'header_descriptor_invariants_invalid')
}
