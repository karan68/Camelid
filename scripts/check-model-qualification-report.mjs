...
function validateCandidateQualification(report, fail) {
  ...
  if (metadata?.status === 'pass') {
    ...
    if (![2, 3].includes(observed.gguf_version)
      ...
      || !Number.isSafeInteger(observed.general_file_type) || observed.general_file_type < 0
      || !safeObserved(observed.declared_quantization)
    ...
    if (!versionMatch
      ...
      || inspector.source_tracked_dirty !== false
