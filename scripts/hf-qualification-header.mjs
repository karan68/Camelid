...
const HEADER_ERROR_CONTRACTS = Object.freeze({
  ...
  header_inspector_changed: ['blocked', 'camelid inspector or tracked source changed during header qualification'],
...
function normalizeInspectorIdentity(identity) {
  ...
  if (!identity || typeof identity !== 'object' || Array.isArray(identity)
    ...
    || typeof identity.source_tracked_dirty !== 'boolean') {
    throw headerError(
  ...
  const derived = {
    ...
    source_tracked_dirty: identity.source_tracked_dirty,
...
async function inspectBinaryIdentity(binary, {
  ...
  try {
    ...
    const [headResult, statusResult] = await Promise.all([
      gitImpl('git', ['rev-parse', 'HEAD'], options),
      gitImpl('git', ['status', '--porcelain', '--untracked-files=no'], options),
    ])
    sourceHead = String(headResult.stdout || '').trim()
    sourceTrackedDirty = Boolean(String(statusResult.stdout || '').trim())
  } catch {
    throw headerError('header_inspector_unavailable', 'blocked', 'current source state could not be read')
  }
...
async function inspectRemoteHeader(lock, {
  ...
  try {
    ...
    let inspectorAfter
    try { inspectorAfter = normalizeInspectorIdentity(await identityImpl(binary, { sourceRoot })) }
    catch {
      throw headerError(
        'header_inspector_changed',
        'blocked',
        'camelid inspector or tracked source changed during header qualification',
      )
    }
    if (JSON.stringify(inspectorAfter) !== JSON.stringify(inspector)) {
      throw headerError(
        'header_inspector_changed',
        'blocked',
        'camelid inspector or tracked source changed during header qualification',
