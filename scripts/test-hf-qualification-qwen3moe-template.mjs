...
expectPackTamper((value) => { value.private_path = 'C:\\Users\\private\\secret.gguf' }, 'private path')
...
const baseDeps = {
  ...
  writeFileImpl: async () => {},
  readFileImpl: async () => Buffer.from(fakePrefix),
...
await assert.rejects(
  qualifyQwen3MoeTemplate(qualificationOptions, {
    ...
    readFileImpl: async () => {
      prefixReads += 1
      return prefixReads === 1 ? Buffer.from(fakePrefix) : Buffer.alloc(PREFIX_BYTES, 1)
    },
  }),
  (error) => error.code === 'qwen3moe_template_prefix_identity_mismatch',
)
