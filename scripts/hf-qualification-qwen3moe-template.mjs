...
const GROUNDING_FILE_SIZE_BYTES = Object.freeze({
  [HEADER_GROUNDING_RECEIPT]: 2_541,
  [TOKENIZER_GROUNDING_RECEIPT]: 13_040,
  [TEMPLATE_GROUNDING_FIXTURE]: 5_897,
})
...
function hardenExecImpl(execImpl = execFileAsync, inheritedEnv = process.env) {
  return (file, args, options = {}) => execImpl(file, args, {
    ...options,
...
async function inspectRegularFileMetadata(path, {
  ...
  catch { throw templateError(unavailableCode) }
  if (linkStats?.isFile?.() !== true || linkStats?.isSymbolicLink?.() !== false) {
    throw templateError(mismatchCode)
  }
...
async function inspectFileIdentity(path, {
  ...
  unavailableCode = 'qwen3moe_template_inspector_unavailable',
  mismatchCode = 'qwen3moe_template_inspector_identity_mismatch',
} = {}) {
  ...
  let digest
  try {
    digest = await sha256FileImpl(path, { expectedSize: fileStats.size, maxBytes })
  } catch {
    throw templateError(unavailableCode)
  }
  if (expectedSha256 !== null && digest !== expectedSha256) {
    throw templateError(mismatchCode)
  }
  return { executable: basename(path), size_bytes: fileStats.size, binary_sha256: digest }
}
...
async function runTemplateAnalyzer(analyzer, template, {
  ...
  inspectFileIdentityImpl = inspectFileIdentity,
  lstatImpl = lstat,
  mkdtempImpl = mkdtemp,
  rmImpl = rm,
  sha256FileImpl = sha256File,
  ...
  const safeExecImpl = hardenExecImpl(execImpl, inheritedEnv)
  ...
  try {
    const analyzerBefore = await inspectFileIdentityImpl(analyzer, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSha256: LLAMA_ANALYZER_SHA256,
    ...
    await inspectFileIdentityImpl(templatePath, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSize: TEMPLATE_UTF8_BYTES,
      expectedSha256: TEMPLATE_SHA256,
      maxBytes: TEMPLATE_UTF8_BYTES,
      unavailableCode: 'qwen3moe_template_identity_mismatch',
      mismatchCode: 'qwen3moe_template_identity_mismatch',
    })
    let analysis = null
    for (let attempt = 0; attempt < 16; attempt += 1) {
      let result
      try {
        result = await safeExecImpl(analyzer, ['--template-file', templatePath], {
          timeout: 30_000,
    ...
    const analyzerAfter = await inspectFileIdentityImpl(analyzer, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSize: analyzerBefore.size_bytes,
      expectedSha256: LLAMA_ANALYZER_SHA256,
      maxBytes: LOCAL_FILE_LIMITS.oracle_file_bytes,
      unavailableCode: 'qwen3moe_template_oracle_changed',
      mismatchCode: 'qwen3moe_template_oracle_changed',
    })
    if (analyzerAfter.binary_sha256 !== analyzerBefore.binary_sha256) {
...
async function inspectCamelid(binary, {
  ...
  let identity
  try {
    identity = await inspectFileIdentityImpl(binary, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSha256: COMMITTED_GROUNDING_INSPECTOR.binary_sha256,
      maxBytes: LOCAL_FILE_LIMITS.binary_bytes,
      unavailableCode: 'qwen3moe_template_inspector_unavailable',
      mismatchCode: 'qwen3moe_template_inspector_identity_mismatch',
    })
    const result = await safeExecImpl(binary, ['--version'], {
      timeout: 10_000,
      maxBuffer: 1024 * 1024,
      windowsHide: true,
    })
...
async function inspectLlamaPackage(analyzer, {
  ...
  for (const name of names) {
    ...
    if (aggregateBytes > LOCAL_FILE_LIMITS.oracle_package_bytes) {
      throw templateError('qwen3moe_template_oracle_identity_mismatch')
    }
    let digest
    try {
    ...
    lines.push(`${name}\t${fileStats.size}\t${digest}\n`)
  }
  ...
  const archiveStats = await inspectRegularFileMetadata(archivePath, {
    lstatImpl,
    statImpl,
    expectedSize: LLAMA_ARCHIVE_SIZE_BYTES,
    maxBytes: LLAMA_ARCHIVE_SIZE_BYTES,
    unavailableCode: 'qwen3moe_template_oracle_unavailable',
    mismatchCode: 'qwen3moe_template_oracle_identity_mismatch',
  })
...
async function inspectOracle(analyzer, {
  ...
  let packageIdentity
  try {
    analyzerIdentity = await inspectFileIdentityImpl(analyzer, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSha256: LLAMA_ANALYZER_SHA256,
    ...
    companionIdentity = await inspectFileIdentityImpl(companion, {
      lstatImpl,
      statImpl,
      sha256FileImpl,
      expectedSha256: LLAMA_CLI_SHA256,
      maxBytes: LOCAL_FILE_LIMITS.oracle_file_bytes,
      unavailableCode: 'qwen3moe_template_oracle_unavailable',
      mismatchCode: 'qwen3moe_template_oracle_identity_mismatch',
    })
    packageIdentity = await inspectLlamaPackage(analyzer, {
      platformInfo,
      lstatImpl,
      statImpl,
      readdirImpl,
  ...
  let output = ''
  try {
    const result = await safeExecImpl(companion, ['--version'], {
      timeout: 10_000,
      windowsHide: true,
    })
...
async function readGroundingSnapshot(root, readFileImpl = readFile, {
  ...
  try {
    for (const [relativePath, expectedHash] of [
      ...
      const expectedSize = GROUNDING_FILE_SIZE_BYTES[relativePath]
...
async function qualifyQwen3MoeTemplate({
  ...
  const lstatImpl = deps.lstatImpl || lstat
  const sha256FileImpl = deps.sha256FileImpl || sha256File
  const statImpl = deps.statImpl || stat
  const inheritedEnv = deps.inheritedEnv || process.env
  const execImpl = hardenExecImpl(deps.execImpl || execFileAsync, inheritedEnv)
  const inspectPrefixFile = deps.inspectPrefixFile || ((path) => inspectFileIdentity(path, {
    lstatImpl,
    statImpl,
    sha256FileImpl,
    expectedSize: PREFIX_BYTES,
    expectedSha256: PREFIX_SHA256,
    maxBytes: PREFIX_BYTES,
    unavailableCode: 'qwen3moe_template_prefix_identity_mismatch',
    mismatchCode: 'qwen3moe_template_prefix_identity_mismatch',
  }))
  let roster
  try {
    if (deps.roster) {
      ...
      const absoluteRosterPath = resolve(root, rosterPath)
      await inspectRegularFileMetadata(absoluteRosterPath, {
        lstatImpl,
        statImpl,
        maxBytes: LOCAL_FILE_LIMITS.roster_bytes,
        unavailableCode: 'qwen3moe_template_source_identity_mismatch',
        mismatchCode: 'qwen3moe_template_source_identity_mismatch',
      })
      roster = JSON.parse(await readFileImpl(absoluteRosterPath, 'utf8'))
    }
  } catch {
    throw templateError('qwen3moe_template_source_identity_mismatch')
  }
  ...
  const groundingBefore = deps.readGrounding
    ...
    : await readGroundingSnapshot(root, readFileImpl, { lstatImpl, statImpl, sha256FileImpl })
  const [inspectorBefore, oracleBefore] = await Promise.all([
    deps.inspectCamelid ? deps.inspectCamelid(binary) : inspectCamelid(binary, {
      execImpl, inheritedEnv, lstatImpl, statImpl, sha256FileImpl,
    }),
    deps.inspectOracle ? deps.inspectOracle(analyzer) : inspectOracle(analyzer, {
      execImpl, inheritedEnv, lstatImpl, statImpl, sha256FileImpl,
    }),
  ])
  let lock
  ...
  try {
    ...
    const prefixBefore = await inspectPrefixFile(prefixPath)
    if (prefixBefore?.size_bytes !== PREFIX_BYTES
      || prefixBefore?.binary_sha256 !== PREFIX_SHA256) {
      throw templateError('qwen3moe_template_prefix_identity_mismatch')
    }
    let inspection
    try {
    ...
    const analysis = deps.runAnalyzer
      ...
      : await runTemplateAnalyzer(analyzer, template, {
        execImpl, inheritedEnv, lstatImpl, statImpl, sha256FileImpl,
      })
    ...
    const [inspectorAfter, oracleAfter, lockAfter, groundingAfter, prefixAfter] = await Promise.all([
      ...
      deps.readGrounding ? deps.readGrounding(root) : readGroundingSnapshot(root, readFileImpl,
        { lstatImpl, statImpl, sha256FileImpl }),
      inspectPrefixFile(prefixPath),
    ])
    ...
    if (prefixAfter?.size_bytes !== PREFIX_BYTES
      || prefixAfter?.binary_sha256 !== PREFIX_SHA256) {
      throw templateError('qwen3moe_template_prefix_identity_mismatch')
    }
