...
async function inspectExactArtifactIdentity(path, {
  ...
  expect(artifactLinkStats.isFile?.() === true
  ...
  expect(artifactStats.isFile?.() === true
  ...
  expect(artifactSha256 === EXACT_ROW.source.sha256,
  'load_smoke_artifact_identity_mismatch')
...
async function readResponseTextBounded(response, controller) {
  const failure = () => (
    controller.signal.reason instanceof SmolLM3LoadSmokeError
  ...
  // The rejection is also consumed by each active Promise.race. This guard
  // covers an abort that lands after the final read but before listener removal.
