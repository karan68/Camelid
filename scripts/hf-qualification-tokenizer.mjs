...
const TOKENIZER_ERROR_CONTRACTS = Object.freeze({
  ...
  tokenizer_oracle_platform_unsupported: ['blocked', 'the pinned tokenizer oracle package is qualified only on Windows x64'],
...
function validIdArray(value, tokenCount, allowEmpty = false) {
...
function tokenizerCaseAllowsEmptyIds(rowId, caseId) {
  return caseId === 'empty_with_add_special'
    && (rowId === 'smollm3_3b_q8_0' || rowId === 'qwen3_30b_a3b_q8_0')
}
...
function assessTokenizerReceiptForPack(receipt, row, defaults, pack, {
  ...
  for (let index = 0; index < pack.cases.length; index += 1) {
    ...
    const allowEmpty = tokenizerCaseAllowsEmptyIds(row.id, expected.id)
    const idsValid = validIdArray(observed.camelid_ids, tokenCount, allowEmpty)
      && validIdArray(observed.llama_cpp_ids, tokenCount, allowEmpty)
  ...
  if (row.id === 'smollm3_3b_q8_0') {
    ...
    check(Boolean(emptyWithSpecial)
      && validIdArray(emptyWithSpecial.camelid_ids, tokenCount, true)
      && validIdArray(emptyWithSpecial.llama_cpp_ids, tokenCount, true)
  ...
  try {
    ...
    try {
      for (const testCase of pack.cases) {
        ...
        const allowEmpty = tokenizerCaseAllowsEmptyIds(row.id, testCase.id)
        if (!validIdArray(camelidResult?.ids, pack.metadataSummary.token_count, allowEmpty)
          || !validIdArray(llamaIds, pack.metadataSummary.token_count, allowEmpty)) {
...
Options:
  ...
  --llama-tokenize <path>  Pinned Windows-x64 llama-tokenize b9632 binary
...
`)
  ...
  if (process.platform !== 'win32' || process.arch !== 'x64') {
    throw tokenizerError('tokenizer_oracle_platform_unsupported')
  }
