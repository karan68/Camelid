...
async function inspectExactArtifactIdentity(path, {
  ...
  try {
    const [linkStats, fileStats, fileSha256] = await Promise.all([
      lstatImpl(path), statImpl(path), sha256FileImpl(path),
    ])
    expect(linkStats.isFile() && linkStats.isSymbolicLink() === false
      && fileStats.isFile()
      && fileStats.size === EXACT_ROW.source.size_bytes
      && fileSha256 === EXACT_ROW.source.sha256,
...
async function runPreflight(options, deps = {}) {
  ...
  let fileStats
  try {
    [linkStats, fileStats] = await Promise.all([
      (deps.lstatImpl || lstat)(options.artifact),
      (deps.statImpl || stat)(options.artifact),
    ])
  } catch {
    throw loadSmokeError('load_smoke_artifact_unavailable')
  }
  expect(linkStats.isFile() && linkStats.isSymbolicLink() === false
    && fileStats.isFile() && fileStats.size === EXACT_ROW.source.size_bytes,
  'load_smoke_artifact_identity_mismatch')
...
function validateLoadSmokeReceiptUnsafe(receipt) {
  ...
  check(receipt?.runtime_contract?.cwd_redacted === true
    ...
    && sameJson(receipt?.runtime_contract?.environment, SAFE_CAMELID_ENV)
