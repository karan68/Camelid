...
const GROUNDING_FILES = Object.freeze({
  ...
  runtime_envelope: Object.freeze({
    ...
    sha256: '9bd93ef2470936ee24f0e740892b8e7bf28967f17d8ae566215a5935b576fee5',
  }),
...
const RENDERER_GIT_BLOB_SHA1 = '3de4cb8602356b6773bcdcbe0c4a69a4155e713f'
...
function buildLlamaEnv(inherited = process.env) {
  ...
  for (const [key, value] of Object.entries(inherited)) {
    ...
    if (!upper.startsWith('CUDA_') && !upper.startsWith('GGML_') && !upper.startsWith('LLAMA_')
      && value !== undefined) clean[key] = value
  }
...
async function sha256File(path, { createReadStreamImpl = createReadStream } = {}) {
...
async function inspectExactArtifactIdentity(path, {
  lstatImpl = lstat,
  statImpl = stat,
  sha256FileImpl = (candidate) => sha256File(candidate),
} = {}) {
  let linkStats
  let fileStats
  let digest
  ...
  expect(linkStats.isFile() && linkStats.isSymbolicLink() === false && fileStats.isFile()
    && fileStats.size === EXACT_ROW.source.size_bytes && digest === EXACT_ROW.source.sha256,
...
async function inspectLlamaPackage(server, {
  ...
  try {
    for (const name of names) {
      ...
      const identity = { size_bytes: bytes.length, sha256: sha256(bytes) }
  ...
  let archive
  try { archive = await readFileImpl(join(dirname(binDir), LLAMA_PIN.archive)) }
  catch { throw parityError('chat_parity_oracle_unavailable') }
  let versionOutput
  try {
  ...
  expect(version && Number(version[1]) === LLAMA_PIN.build
    ...
    && archive.length === LLAMA_PIN.archive_size_bytes
    && sha256(archive) === LLAMA_PIN.archive_sha256,
  'chat_parity_oracle_identity_mismatch')
  return {
...
async function inspectGroundings(root, {
  ...
  for (const [name, identity] of Object.entries(GROUNDING_FILES)) {
    let bytes
    try { bytes = await readFileImpl(resolve(root, identity.path)) }
    catch { throw parityError('chat_parity_grounding_invalid') }
    expect(sha256(bytes) === identity.sha256, 'chat_parity_grounding_invalid')
...
async function engineProcessesPresent({ execFileImpl = execFileAsync } = {}) {
  try {
...
async function runPreflight(options, deps = {}) {
  ...
  } catch { throw parityError('chat_parity_artifact_unavailable') }
  expect(linkStats.isFile() && linkStats.isSymbolicLink() === false && fileStats.isFile()
    && fileStats.size === EXACT_ROW.source.size_bytes,
  'chat_parity_artifact_identity_mismatch')
