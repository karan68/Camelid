...
await pendingRead
requestController.abort(new Qwen25LoadSmokeError('load_smoke_resource_abort'))
await expectCode(cancelledRequest, 'load_smoke_resource_abort')
...
for (const { name, linkStats, identityStats, expectedStatCalls } of [
  ...
  },
  {
    name: 'wrong-size',
    linkStats: fileStats,
    identityStats: { ...fileStats, size: EXACT_ROW.source.size_bytes - 1 },
    expectedStatCalls: 1,
  },
]) {
  let statCalls = 0
