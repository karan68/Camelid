...
const inspection = {
  ...
  metadata: {
    ...
    'upstream.private.note': ['', 'Users', 'operator', 'model.gguf'].join('/'),
...
maliciousObservedInspection.metadata['tokenizer.ggml.model'] = [
  '', 'Users', 'operator', 'tokenizer.json',
].join('/')
...
const inspectedIdentity = await inspectBinaryIdentity('<camelid>', {
  ...
  gitImpl: async (binary, args, options) => {
    ...
    ].some((expected) => JSON.stringify(args) === JSON.stringify(expected)))
    assert.equal(options.cwd, '<source-root>')
    return { stdout: args[0] === 'rev-parse' ? `${sourceHead}\n` : '' }
  },
...
const dirtyInspectedIdentity = await inspectBinaryIdentity('<camelid>', {
  ...
  gitImpl: async (_binary, args) => ({ stdout: args[0] === 'rev-parse' ? `${sourceHead}\n` : '' }),
})
...
const trackedDirtyIdentity = await inspectBinaryIdentity('<camelid>', {
  execImpl: async () => ({ stdout: 'camelid v0.6.1-27-g12345678\n' }),
  hashImpl: async () => 'e'.repeat(64),
  gitImpl: async (_binary, args) => ({
    stdout: args[0] === 'rev-parse' ? `${sourceHead}\n` : ' M src/model.rs\n',
  }),
})
assert.equal(trackedDirtyIdentity.source_tracked_dirty, true)
assert.equal(trackedDirtyIdentity.clean_current_head, false)
await assert.rejects(
...
await assert.rejects(
  inspectRemoteHeader(lock, {
    ...
    identityImpl: async () => {
      identityReadCount += 1
      return identityReadCount === 1
        ? inspectorIdentity
        : { ...inspectorIdentity, source_tracked_dirty: true, clean_current_head: false }
    },
    fetchImpl: async () => new Response(Buffer.alloc(16, 11), {
      status: 206,
      headers: { 'content-range': 'bytes 0-15/100' },
    }),
    inspectImpl: async () => inspection,
  }),
  (error) => error instanceof HeaderInspectionError
    && error.code === 'header_inspector_changed'
    && error.status === 'blocked',
)
assert.equal(identityReadCount, 2, 'header qualification must re-read inspector/source identity')
