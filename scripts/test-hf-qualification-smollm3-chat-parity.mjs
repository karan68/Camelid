...
assert.equal(GROUNDING_FILES.runtime_envelope.sha256,
  '9bd93ef2470936ee24f0e740892b8e7bf28967f17d8ae566215a5935b576fee5')
...
const childEnv = buildChildEnv({ PATH: 'kept', CAMELID_PROFILE: 'fast',
  CAMELID_SECRET: 'removed', CUDA_VISIBLE_DEVICES: '7' })
...
const llamaEnv = buildLlamaEnv({ PATH: 'kept', CUDA_VISIBLE_DEVICES: '7',
  GGML_BACKEND: 'cuda', LLAMA_ARG: 'unsafe', CAMELID_PROFILE: 'kept-for-non-camelid-child' })
assert.equal(llamaEnv.PATH, 'kept')
assert.equal(llamaEnv.GGML_BACKEND, undefined)
...
const tamperCases = [
  ...
  ['absolute path', (value) => { value.provenance.camelid_binary.profile = 'C:\\Users\\private\\x' },
    'absolute local path'],
  ['credential', (value) => { value.provenance.camelid_binary.profile = 'ghp_1234567890abcdefghijklmnop' },
    'credential'],
