...
- **Acceptance target:** Llama 3.2 3B Instruct Q8_0 is the exact next WebUI real-chat target. The exact tracked GGUF is present locally, `/api/models/load` succeeds with low backend RSS after streaming metadata parsing, and fresh Ubuntu backend-only artifacts now cover repeat 5-token generation plus a bounded 50-token run. This is backend evidence only: no support, prompt-token parity, broader prompt/chat-template parity, or WebUI claim should be inferred until exact-row promotion evidence exists.
- **Groundwork-only lane:** Llama 3 8B Instruct Q8_0 has metadata/config/tokenizer/template evidence, independent tokenizer reference fixtures, a materialization-budget guard, lazy/file-backed Q8 execution work, repeat bounded Ubuntu backend-only 5-token generation, prompt-cache validation, a fresh 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and a bounded memory gate on the exact tracked Q8_0 GGUF. It still remains below supported generation until the row has broader prompt/chat-template parity, WebUI readiness, and performance/portability evidence.
...
In plain terms: TinyLlama Q8_0 is the live supported gate; Llama 3.2 1B is a narrow evidence row; Llama 3.2 3B is the chosen next acceptance target and now has repeat backend-only short generation plus a bounded 50-token artifact; and Llama 3 8B is still groundwork only in release terms, but now has repeat backend-only 5-token generation, prompt-cache validation, a 50-token backend/API artifact, raw prompt-token parity, API-smoke, and memory evidence of its own.
...
| Llama 3.2 3B Instruct Q8_0 | Acceptance target / backend evidence only | The exact tracked GGUF loads through `/api/models/load`, the lazy/file-backed Q8 seam is partially wired, and Ubuntu backend-only artifacts now include repeat 5-token generation plus a bounded 50-token run for `hello`. | Add prompt-token parity, broader prompt/chat-template parity, API smoke, WebUI readiness, and stronger memory/performance follow-up evidence. |
| Llama 3 8B Instruct Q8_0 | Groundwork only / backend evidence only | Fixture-guarded metadata/template handling exists, tokenizer reference parity is guarded, and repeat bounded Ubuntu backend-only 5-token generation plus prompt-cache validation, a 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and bounded memory evidence now exist. | Add broader prompt/chat-template parity, WebUI readiness, performance evidence, and portable packaging evidence before any support promotion. |
...
- **Family / quant:** LLaMA decoder + Llama 3 BPE, Q8_0
- **Validated now:** the exact tracked GGUF is present, metadata/API load is validated, the file-backed lazy-Q8 seam is partially wired, and Ubuntu backend-only artifacts now include repeat 5-token generation plus one bounded 50-token run for `hello`
- **Missing gates:** prompt-token parity, broader prompt/chat-template parity, API smoke, WebUI readiness, and stronger memory/performance follow-up evidence
- **Promotion blocker:** support remains frozen until Camelid has exact-row prompt-token/parity/API/WebUI evidence that matches the stronger backend generation wins

#### Llama 3 8B Instruct Q8_0
- **Family / quant:** LLaMA decoder + Llama 3 BPE, Q8_0
- **Validated now:** metadata/config/template handling is fixture-guarded, tokenizer reference parity is guarded, lazy/file-backed Q8 execution reached repeat bounded Ubuntu backend-only 5-token generation for `hello`, raw `hello` prompt-token parity is captured for the exact model SHA, prompt-cache validation plus a 50-token backend/API artifact exist, basic API smoke passed, and the current-head memory gate stayed bounded
...
For **Llama 3.2 3B** specifically, tracked local model presence, repeat bounded short generation, and one bounded 50-token backend run are now satisfied. The next promotable evidence is prompt-token parity plus API and WebUI smoke for the exact row before any support promotion.

For **Llama 3 8B** specifically, the next promotable evidence is not another tokenizer freshness pass or standalone `bench-q8-blocks` report. Camelid now has repeat bounded backend-only 5-token generation, prompt-cache validation, a 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and bounded memory evidence for the exact row. The next promotable evidence is broader prompt-pack/chat-template parity, WebUI readiness, performance, and portability artifacts for that same row.
...
- **Phase 11 — Llama 3 / Backend + QA:** keep Llama 3 below support until each concrete target has the right artifact-backed evidence. For Llama 3.2 3B Q8_0, exact GGUF presence plus repeat backend-only short generation and one bounded 50-token artifact are now satisfied; the next requirement is prompt-token/API/WebUI acceptance evidence from `QA_LLAMA32_3B_Q8_ACCEPTANCE.md` without borrowing from the 1B or 8B rows. For Llama 3 8B, repeat bounded backend-only 5-token generation, prompt-cache validation, a 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and bounded memory evidence now exist, but the exact next Backend/Performance action is still to extend this into broader prompt/chat-template parity, WebUI readiness, and performance evidence, preserving the serial/block-aligned determinism guardrails until optimized kernels have separate zero-delta evidence.
