...
For **Llama 3.2 3B** specifically, tracked model presence, exact-row load success, compact prompt-token/1-token/5-token/50-token parity, API smoke, WebUI smoke, and a five-prompt API smoke pack are now satisfied for the short smoke-supported row. The downloaded-model matrix also confirmed the broader JSON-shaped prompt still diverges despite matching prompt tokens, so the next promotable evidence remains broader prompt/chat-template parity, longer contexts, and stronger performance/portability evidence before expanding the support claim.
...
| LLaMA decoder + Llama 3 BPE | Exact 1B/3B smoke supported; 8B groundwork only | The Llama 3.2 1B Instruct Q8_0 row has compact parity, the current three-prompt broader prompt-pack parity, API smoke, and frontend smoke. The Llama 3.2 3B Instruct Q8_0 row has exact load, compact prompt-token/1-token/5-token/50-token parity, API smoke, and frontend smoke, but its broader JSON-shaped prompt still diverges. The Llama 3 8B Instruct Q8_0 artifact has tokenizer/config/template fixtures, compact parity, basic API smoke, bounded memory evidence, and partial downloaded-matrix passes, but remains below support. Broader Llama 3 support, longer context, full chat-template behavior, performance, and portable packaging remain unsupported until separately scoped. |
...
- **Phase 11 — Llama 3 / Backend + QA:** keep broad Llama 3 below support until each concrete target has the right artifact-backed evidence. For Llama 3.2 1B Q8_0, exact-row short-chat smoke support is now strengthened by a passing three-prompt llama.cpp pack in the downloaded-model matrix. For Llama 3.2 3B Q8_0, exact-row short-chat smoke support is satisfied by compact parity, API, frontend, and smoke-pack evidence, but the downloaded-model matrix reconfirmed the broader JSON-shaped prompt divergence, so broader prompt/chat-template parity remains the next requirement before widening the row. For Llama 3 8B, compact prompt-token, 1-token, 5-token, and bounded 50-token parity, basic API smoke, bounded memory evidence, and partial downloaded-matrix passes now exist, but the exact next Backend/Performance action is still to rerun the alpacas prompt with a longer backend client timeout and then extend this into broader prompt/chat-template parity, WebUI readiness, and performance evidence, preserving the serial/block-aligned determinism guardrails until optimized kernels have separate zero-delta evidence.
