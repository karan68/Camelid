...
- **Phase 11 — Llama 3 / Backend + QA:** keep broad Llama 3 below support until each concrete target has the right artifact-backed evidence. For Llama 3.2 1B Q8_0, exact-row short-chat smoke support is now strengthened by a passing three-prompt known-good-baseline pack in the downloaded-model matrix, the first bounded 512-context pack, and the second bounded 1024-context pack. For Llama 3.2 3B Q8_0, exact-row smoke support is now strengthened by compact parity, API, frontend, smoke-pack evidence, the post-Q8-dot clean broader three-prompt 50-token parity rerun, the first bounded 512-context pack, the second bounded 1024-context pack, and the third bounded 2048-context pack; model-native/larger-context, performance/portability, and broader chat-template evidence remain the next requirements before widening the row. For Llama 3 8B, compact prompt-token, 1-token, 5-token, bounded 50-token parity, the three-prompt 50-token broader pass, basic API/frontend smoke, the first bounded 512-context pack, and bounded memory evidence now support the exact row; the attempted 2048-context pack is explicitly blocked by Camelid backend non-completion within the 900-second parity harness timeout. The next Backend/Performance action is to run the separate 1024-context pack, debug the 2048-timeout path, broaden prompt-pack/template coverage, and close performance/portability evidence, preserving the serial/block-aligned determinism guardrails until optimized kernels have separate zero-delta evidence.
