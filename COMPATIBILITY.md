...
- **Family / quant:** LLaMA decoder + Llama 3 BPE, Q8_0
- **Validated now:** the exact tracked GGUF is present, metadata/API load is validated, the file-backed lazy-Q8 seam is partially wired, and Ubuntu backend-only artifacts now include repeat 5-token generation plus a bounded 50-token run for `hello`
