...
- **`load.quant_breadth`** — (SECONDARY) loadability across additional quants beyond Q8_0
  - Q8_0 supported; broader K-quant decode (Q4_K/Q5_0/Q6_K) gated/experimental (wire_dequant.rs). SECONDARY axis.
    ...
    - **UPDATE (BASALT, 2026-07-17; GABBRO M2, 2026-07-18):** NVFP4 (4-bit) is now decodable/loadable as engine facts for the **gemma-4-E4B pilot only, on Windows and macOS** — bit-exact CPU decode, validated on x86 and on Apple Silicon/ARM (GABBRO Gate G-M1), + a Windows CUDA-resident `nvfp4_gemv` dp4a kernel (46/46 bit-identical to the CPU oracle), with sidecar-bearing and NaN-sentinel files failing closed. On macOS only the CPU wire lane runs NVFP4; the Metal GPU kernel is GABBRO Phase M3 (not yet wired — `Gemma4GpuRuntime` still typed-refuses NVFP4). Targets other than Windows/macOS refuse with the typed TK2 error. It is NOT a supported/certified row and NOT quality-competitive: measured vs the Q8_0 parent at matched 4.5 bpw it is behind Q4_K (Gate G3 NO-GO, 88.5% vs 92.6% top-1 agreement, 0.111 vs 0.065 mean-KL nats), while on an RTX 3060 Laptop the CUDA decode is 1.03x faster than Q8_0 (26.51 vs 25.80 tok/s) and 2.08 GB lighter in VRAM (Gate G4, decode-only, Windows/this box — no macOS perf claim). Registered as a `planned_quantization` axis item only (no `model_compatibility` row, no frontend catalog); spec `docs/architecture/NVFP4_FORMAT.md`; macOS receipt `qa/evidence-bundles/gabbro/phase1/`. This is a `load.quant_breadth` (format-breadth) note, not a capability-feature row.
