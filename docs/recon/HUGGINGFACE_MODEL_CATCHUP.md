...
| SmolLM3 | Source, clean-head bounded exact-row metadata, the 10-case `smaug-bpe` tokenizer pack, and a guarded exact-row Windows CPU load smoke pass. The 3,275,574,624-byte Q8_0 artifact loaded on the Safe `cpu_reference` lane; the raw first forward and bounded default-thinking chat follow-up each emitted one finite greedy token. All 254 Q8 tensors remained lazy and file-backed with zero Q8-to-F32 materialization or retained blocks, and CUDA remained disabled. A separately guarded [4-token chat-parity-preparation receipt](../../qa/model-qualification/smollm3-3b-q8-windows-cpu-chat-parity-preparation.json) records exact prompt token IDs, generated token IDs, and detokenized text against pinned llama.cpp b9632 for one default-thinking text case. | This preparation evidence does not advance roster parity: the 1/5/50-token matrix remains blocked. The full template gate remains blocked outside the narrow default-thinking text envelope, while API/WebUI/SSE and 512-token context evidence remain pending. The receipt makes no broader correctness, performance, GPU, support, adjacent-row, tools, system, no-think, multimodal, streaming, or non-text claim. |
