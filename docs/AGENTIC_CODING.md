...
For a live model check, start an engine with an isolated `CAMELID_WORKSPACE_MEMORY_DB`, then run from `frontend`: `CAMELID_CODING_LIVE_URL=http://127.0.0.1:18191 node scripts/coding-live-smoke.mjs`. It creates a temporary Python fixture and accepts only the expected file edit and exact test command. The check requires two completed helpers, a successful test result, an idempotent create retry, and successful Undo. It saves observations under `target/coding-live`.
