...
- `src/chat/workspace_memory.rs` owns an app-managed bundled SQLite/FTS5 schema-v4 store under `%LOCALAPPDATA%\camelid\workspace-memory.sqlite3` (or the platform data-directory fallback). `CAMELID_WORKSPACE_MEMORY_DB` overrides the path for isolated tests and controlled deployments.
...
- Saved conversations are listed only for a caller-supplied canonical folder. Resume is explicit in the UI; a new conversation remains the default. Resume verifies canonical root, original model ID, and the exact loaded GGUF SHA-256.
...
- Model streaming uses the turn-scoped cancellation flag rather than the process-global CLI flag. A 90-second model-step deadline covers both the wait for HTTP headers and the SSE body. Cancellation is rechecked after every model step before any partial answer can be reported or persisted, and cancellation state is preserved if DELETE races event-stream claim or worker completion.
...
- Immediate non-recursive extension inventories are deterministic after `list_dir`: matching filenames are case-folded for deduplication, sorted, rendered as stable bullets, and accompanied by explicit directory/nesting scope. Model prose cannot add directory entries, reverse a positive observation, or fabricate the empty result.
...
- Final-artifact cancellation receipt: `target/workspace-memory-final-cancel-races/summary.json`. DELETE after a real `model.delta` produced `aborted` in 21 ms, left terminal state `cancelled`, and persisted no partial turn. DELETE before the event stream was claimed produced `aborted` in 26 ms, remained `cancelled`, and also persisted no turn. A focused loopback socket test covers the pre-header model-step deadline; a focused agent-loop test proves cancellation during a model step discards partial text. The newest API state-unit executable was compiled but locally blocked before execution by Windows Application Control; the release-path race receipt passed.
