...
- `src/chat/workspace_memory.rs` owns an app-managed bundled SQLite/FTS5 schema-v5 store under `%LOCALAPPDATA%\camelid\workspace-memory.sqlite3` (or the platform data-directory fallback). `CAMELID_WORKSPACE_MEMORY_DB` overrides the path for isolated tests and controlled deployments.
