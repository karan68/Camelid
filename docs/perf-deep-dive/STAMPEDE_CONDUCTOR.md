...
| 6 | CUDA multi-stream | GPU decode +10–15% | M | none (no re-assoc) | **DESIGNED 2026-07-09, impl queued** — full turnkey plan in `PHASE6_CUDA_STREAMS_PLAN.md` (K/V + FFN-up on side streams, 5 events/layer, `CAMELID_CUDA_STREAMS` default-off, LIVE path only — capture stays single-stream per cudarc/WDDM). Bitwise-neutral by construction. Deferred from this session: the launch-site threading is delicate hot-path work needing on-GPU iteration, and the design's own biggest risk is a NULL result on this WDDM laptop (cudarc multi-stream event-tracking tax + sub-100µs scheduling) — measure first, then `disable_event_tracking`, then honest NULL receipt if it doesn't move. |
