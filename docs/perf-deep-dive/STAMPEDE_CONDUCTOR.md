...
| 5 | Model-free CPU spec decode | decode 1.3–2× (workload-dep) | M | none (lossless verify) | **Stage A LANDED 2026-07-09, economics KILLED at current verify cost (ships default-off)** — P5.1 CPU chunk-verify + rollback wired into the spec-tree lane (primary-chain flatten, one-way resident ratchet, `CAMELID_SPEC_CPU_VERIFY=0` kill-switch); P5.2 latch extracted to `speculative::SpecLatch` (5 unit tests); LOSSLESS on all 5 workload cells; acceptance PROVEN (5.45 drafts/round on repetitive, 64%); blocker: 8-row chunk verify costs **7.1× a decode step** (premise ≲3× from large-M receipts — small-M amortization is ~nil), capping even perfect acceptance at 1.13×. Follow-up slice: profile/fix the M∈[4,8] chunk path — at ~3× the gate flips to GO (~2.1× repetitive available). `stampede-p5-cpu-spec-verdict-20260709.md` |
