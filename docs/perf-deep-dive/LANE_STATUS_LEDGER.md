...
- **Tree-speculation WIDTH on Apple Silicon (BARCHAN Gate 1, 2026-07-20, M4 / 3B Q8_0):** per-round
  verify cost is **linear in tree width** at **40.18 ms per node** against a **38.0 ms** plain decode
  step — i.e. each verified row costs a full independent decode, and the batched verify amortizes the
  weight read **not at all**. k=15 vs k=1 = **9.55×** (KILL threshold 3×). Fixed per-round cost
  ...
  s_sync ≈ 1.07). Phases 3–4 dropped. Full curve: `BARCHAN_PHASE1_COST_CURVE.md`.
