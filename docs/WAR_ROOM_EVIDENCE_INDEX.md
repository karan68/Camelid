...
- `/metrics` may be cited as Camelid operational telemetry only when focused tests prove Prometheus content type/shape, bounded metric names, no model/prompt/path/secret/user labels, authentication behavior, and real engine/RSS/VRAM gauge wiring. It must not imply llama-server metric-name parity, continuous-batching promotion, model support, benchmark throughput, or WebUI readiness.
