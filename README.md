...
| Llama 3.2 3B Instruct | Q4_K_M, Q5_K_M | **GPU-resident raw-decode parity, held out of the runtime support gate** — `/api/capabilities` does not advertise these yet (recognition pending API/WebUI/serve smoke). GPU-resident CUDA decode drives the `q4k_gemv`/`q5k_gemv` + `q6k_gemv` kernels on the mixed K-quant tensors, direct against llama.cpp (no camelid CPU K-quant decode path). No speed, neighboring-quant, or family claim. | Q5_K_M: token-and-text-identical to pinned llama.cpp `acd79d603` at 1/5/50 tokens (`all_pass=true`). Q4_K_M: token-and-text-identical on 5/8 confident probes at 1/5/50 (incl. code completion + a long-context continuation to depth 50); 3 open-ended probes diverge at benign greedy f32 near-ties. Bundles under `qa/evidence-bundles/llama-3.2-3b-q{4,5}_k_m-windows-cuda-resident-parity-*/` |
...
| Other quantizations | ⛔ Not supported | No supported row; implemented architectures may load in the experimental lane (unverified, no parity claim), everything else fails closed. |
