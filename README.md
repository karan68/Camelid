...
Fresh reopened-lane API-only smoke: `qa/evidence-bundles/four-row-api-only-20260504T230722Z-head-13a465608fbf/manifest.json` records passing exact-row `/api/models/load`, `/v1/models`, `/api/capabilities`, `/v1/completions`, and `/v1/chat/completions` checks for TinyLlama, Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Q8_0. The separate first 8B 512-context rerun at `qa/evidence-bundles/llama3-8b-context-512-20260504T234625Z-head-58acf592345c/manifest.json` now passes, but it is one bounded pack only, not full parity/performance evidence.
