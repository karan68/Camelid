...
|---|---|---|---|
| DiffusionGemma 26B-A4B-It | Q4_K_M | **Active development — recon/evidence-only. Not supported.** | [Phase 0 recon](docs/recon/DIFFUSIONGEMMA_RECON.md) (tensor inventory, pinned llama.cpp diffusion reference, sampler recon) + Phase 0.5 bit-exact lazy dequant parity + Phase 1 tokenizer parity (12/12, 100% token-id match) + Phase 2 encoder checkpoint parity **BIT-EXACT at zero tolerance** (242/242 checkpoints, 510/510 expert selections) + Phase 3 single denoise step parity **BIT-EXACT at zero tolerance** (all 67,108,864 canvas logits, full host-RNG streams, every Entropy-Bound step-0 output) + Phase 4 full EB denoise-loop parity **BIT-EXACT at the default parameters** (S=48 with live self-conditioning; the reference's adaptive stop fired at step 7 and camelid matched the whole trajectory, 268M logits values bit-checked) vs the pinned reference's CPU-pure configuration; single-canvas denoise parity only — no chat/serve path, multi-canvas loop = Phase 5 |
