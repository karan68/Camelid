...
- **The context ladder is explicit.** The four supported Llama/TinyLlama rows have checked bounded 512-context evidence; Llama 3.2 1B/3B and Llama 3 8B also have checked 1024 and 2048 packs. Mixtral has only bounded one-token backend MoE runtime evidence; no frontend/API/WebUI/RSS or longer-context bucket is promoted for Mixtral.
...
- **Active next-model bring-up:** Mistral 7B Instruct is the lead exact-row bring-up lane; Qwen 2.5 7B Instruct and Gemma 2 9B Instruct remain planned exact-row candidates.
  ...
  - `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` — bounded backend MoE runtime evidence only: source/SHA/license, tokenizer/template references, lazy/file-backed rank-3 Q8 expert routing, selected top-k weight renormalization, and bounded one-token smoke are documented. Later short-prompt generation still diverges from llama.cpp, and API/WebUI/RSS, frontend readiness, long-context, production, neighboring-row, and broader/full support remain unclaimed.
...
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | **Bounded backend MoE runtime validation** | Source/SHA/license, tokenizer/template references, lazy/file-backed rank-3 Q8 expert routing, selected top-k weight renormalization, and bounded one-token smoke evidence. Key bundle: `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-support-probe-20260509/manifest.json`. | Not broad Mixtral support: later short-prompt generation still diverges from llama.cpp; no frontend/API/WebUI/RSS green state, long-context support, production throughput, neighboring-row, or full-support claim. |
...
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | Bounded backend MoE runtime probe: one-token smoke with selected-weight renormalized top-k routing and lazy/file-backed rank-3 Q8 experts | One-token evidence only; later short-prompt generation still diverges from llama.cpp | Backend runtime validation only; no frontend/API/WebUI/RSS, long-context, or broad support claim |
...
- The next major 8B, Mistral, and Mixtral gaps are broader context, arbitrary templates, production throughput, portability, repeated durability evidence, and normalized support bundles; Mistral still needs row-specific API/WebUI/RSS readiness, and Mixtral first needs later-generation parity before API/WebUI/RSS, long-context, or broader support evidence can widen the claim.
