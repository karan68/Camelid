...
| **Gemma 4 E2B-It** | Q8_0 | single-node (CPU + Metal) | 5/5 greedy parity (CPU + Metal) vs pinned llama.cpp 5d56eff **+ bounded context 512→8192** (committed context-pass bundle, token+text-identical at every bucket) |
...
|---|---|---|
| **Certified per exact row** | Q8_0 (most rows) · Q4_K_M (Qwen3 4B, Llama 3.2 3B, Ornith 9B) · Q5_K_M (Llama 3.2 3B) · Q3_K_M (Ornith 9B) · Q4_0 QAT (Gemma 4 26B-A4B distributed, E4B Metal) · TQ2_0 (Ternary Bonsai 4B, CPU smoke) · the DiffusionGemma Q4_K_M lane | Parity-certified against a pinned reference, listed in the table above with its evidence bundle. Support never spreads to neighboring files or quants. |
| **Runnable/experimental lane** | F32, F16, Q8_0, Q4_0, Q3_K, Q4_K, Q5_K, Q6_K (for the covered architectures) | The engine runs the file in a clearly-marked experimental lane — every reply flagged unverified, **no parity claim** (`src/runnable/admit.rs`). |
| **Load-only** | Q4_1, Q5_0, Q5_1, Q2_K, Q8_K, BF16, IQ4_NL, TQ1_0 (and TQ2_0 outside the certified Bonsai row) | CPU dequant-to-f32 exists (some also have GPU kernels) — an engine fact, not a support claim; loadable for implemented architectures in the experimental lane only. |
