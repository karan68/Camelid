...
- **Active next-model bring-up set:** Mistral 7B Instruct v0.3 is the first next-family exact row under active validation, but it is not supported yet. Camelid is also publicly working on **Mixtral 8x7B Instruct**, **Qwen 2.5 7B Instruct**, and **Gemma 2 9B Instruct** as planned exact-row candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — active validation only: source/SHA and bounded Ubuntu bring-up artifacts exist, but support remains fail-closed pending an exact tokenizer/template reference pack, 1-token generation parity evidence, broader prompt coverage, API/WebUI/RSS evidence, scrubbed bundle checksums, and verified GGUF size/SHA. No Mistral-family, neighboring-row, 50-token, arbitrary-template, production-throughput, portability, or full-support claim is implied.
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | **In active validation; not supported yet** | Source/SHA and bounded Ubuntu bring-up artifacts exist. The first promotable box is still an exact tokenizer/template reference pack plus 1-token generation parity evidence; broader 50-token parity is not green. | No Mistral-family support, neighboring variants, generation readiness, 50-token parity, arbitrary templates, model-native/larger context, API/WebUI readiness, production throughput, portability, or full support. |
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Active validation: Ubuntu bounded bring-up; exact tokenizer/template reference pack, 1-token generation parity, and broader 50-token parity follow-up still required | BLOCKED | First promotable Mistral box is still missing; no support claim |
...
- The next major 8B and Mistral support gaps are fresh current-head 8B 1024/2048 PASS evidence with docs/API/frontend alignment, model-native/larger context beyond checked packs, arbitrary templates, production throughput, portability, broader prompt/token coverage, repeated durability evidence, and normalized full-support bundles.
