...
- **New Llama 3-family evidence, not a support expansion:** Camelid now has stronger exact-row evidence for tracked Llama 3-family lanes, but no Llama 3-family lane is supported today.
  ...
  - **3B acceptance target / backend evidence only:** the exact tracked Llama 3.2 3B Instruct Q8_0 GGUF now loads successfully through `/api/models/load` with low backend RSS after streaming metadata parsing plus lazy/file-backed Q8 handling, and the exact row now has repeat bounded backend-only generation on Ubuntu including fresh 5-token and 50-token artifacts. This is exact-row blocker reduction, not a support promotion, and the row remains blocked on prompt-token parity, broader prompt/chat-template parity, API/WebUI readiness, and promotion-quality evidence.
  - **8B groundwork / backend evidence only:** Llama 3 8B Instruct Q8_0 now has repeat bounded Ubuntu backend-only 5-token generation, prompt-cache validation, a fresh 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and bounded memory evidence on the exact tracked Q8_0 GGUF, on top of the earlier tokenizer/config/Q8 groundwork and lazy/file-backed execution work.
- **Public support contract stays unchanged:** 1B remains evidence-only, 3B remains a blocked acceptance target with backend-only evidence, 8B remains groundwork-only with backend-only evidence, and no Llama 3-family row is a supported generation lane today.
...
| Llama 3.2 3B Instruct Q8_0 | Acceptance target / backend evidence only | The exact tracked GGUF is present locally; that exact GGUF now succeeds through `/api/models/load` with low backend RSS after streaming metadata parsing; file-backed lazy-Q8 materially reduced the earlier eager dense-load spike; and Ubuntu backend-only `/v1/completions` artifacts now include repeat 5-token success plus a bounded 50-token success for `hello`. | Not a supported row. No 3B prompt-token parity, broader prompt/chat-template parity, API readiness, or WebUI readiness claim until bounded exact-row promotion evidence exists. |
| Llama 3 8B Instruct Q8_0 | Groundwork only / backend evidence only | Metadata, tokenizer/config fixtures, independent tokenizer references, lazy/file-backed Q8 execution groundwork, repeat bounded Ubuntu backend-only 5-token generation, prompt-cache validation, a fresh 50-token backend/API artifact, raw `hello` prompt-token parity, basic API smoke, and bounded memory evidence now exist. | No supported generation, broader prompt/chat-template parity, WebUI readiness, performance, or portable-packaging claim until exact-row follow-up evidence exists. |
