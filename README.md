...
| Gemma 4 (E4B) | Experimental | From-scratch `gemma4` engine — per-layer-type sliding/global attention, QK-norm, dual-θ RoPE, GeGLU, Per-Layer-Embeddings, cross-layer KV sharing. E4B-It Q8_0 is validated token-for-token against the reference and served behind `CAMELID_GEMMA4_SERVE`; a Metal GPU-resident decode path (`camelid gemma4-generate-gpu`) runs the full forward on the GPU at the memory-bandwidth wall. Larger dense/MoE `gemma4` variants share the code but are untested. |
