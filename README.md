...
**Requires a tool-capable supported row.** Agent mode is gated to models the ledger marks `tool_capable`, promoted only with a real tool-call round-trip as evidence — the same bar as the support gate. The engine *does* render tool definitions (through each model's own chat template) and the loop parses the tool-call format back out — that plumbing ships and is tested. **But no supported row is marked tool-capable yet:** a live round-trip showed the small Llama 3.2 models emit the right tool-call *shape* with **malformed arguments**, which isn't a usable call. So `--agent` is built and tested (the loop, the full tool set, the sandbox, and the approval gate ship with unit tests) but **refuses every model** with a typed error until a model demonstrably tool-calls cleanly (the more-capable rows are the candidates). This is an *interface*, not a capability claim about any model.
