...
| **Runnable/experimental lane** | F32, F16, Q8_0, Q4_0, Q3_K, Q4_K, Q5_K, Q6_K, IQ4_XS (for the covered architectures); NVFP4 (**gemma4-E4B pilot only, Windows-only** — narrower than the other formats here) | The engine runs the file in a clearly-marked experimental lane — every reply flagged unverified, **no parity claim** (`src/runnable/admit.rs`). NVFP4 admission is a pilot-model carve-out and fails closed on non-Windows, on sidecar-bearing files, and on NaN-sentinel scale bytes — but its produced pilot file (gemma4-E4B) runs via `gemma4_runtime`, not this lane's serve path, and refuses generic runnable admission on its BF16 tensor (G2 §6b). |
...
| Gemma 4 E4B-It | NVFP4 (mm; gemma4-E4B pilot) | **Pilot lane (gemma4 wire + CUDA), Windows-only — receipted engine facts, NOT a supported row, not quality-competitive.** NVFP4 4-bit weights, gemma-4-E4B pilot, Windows-only. Engine facts: bit-exact CPU decode + CUDA dp4a GEMV kernel (46/46 bit-identical) — but its produced pilot file (gemma4-E4B) runs via `gemma4_runtime`, not this lane's serve path, and refuses generic runnable admission on its BF16 tensor (G2 §6b). Measured vs the Q8_0 parent at matched 4.5 bpw: behind Q4_K on quality (G3 NO-GO, 88.5% vs 92.6% top-1 agreement; 0.111 vs 0.065 mean-KL nats), but 1.03x faster than Q8_0 CUDA decode (26.51 vs 25.80 tok/s) and 2.08 GB lighter VRAM on an RTX 3060 Laptop (decode-only, this box). Not a supported row; not quality-competitive. Sidecar-bearing and NaN-sentinel files fail closed; Phase 5 (Blackwell) BLOCKED-HW. | G3 quality (NO-GO): `qa/evidence-bundles/basalt/phase3/BASALT_G3_SUMMARY.md`; G4 CERT + dp4a perf: `qa/evidence-bundles/basalt/phase4/cert/BASALT_G4_SUMMARY.md`; unit gate `nvfp4_gemv_matches_oracle` (46/46 bit-identical); spec `docs/architecture/NVFP4_FORMAT.md` |
