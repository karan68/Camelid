...
1. **Resolved 2 Critical CUDA Initialization Blockers ([PR #669](https://github.com/timtoole02/Camelid/pull/669))**:
   - **Tensor Decoupling Bug ([`src/tensor/mod.rs`](file://./src/tensor/mod.rs))**: Fixed an issue where Q8_0 runtime storage optimization set `q8_0_blocks = None` on x86_64, preventing CUDA resident VRAM uploads from extracting quant bytes.
   - **NVRTC Missing Header Intrinsics Bug ([`src/cuda_resident.rs`](file://./src/cuda_resident.rs))**: Added inline device PTX assembly intrinsics for `__dp4a` (`dp4a.s32.s32`) and `__byte_perm` (`prmt.b32`) to eliminate `CUDA_ERROR_INVALID_PTX` failures in standalone NVRTC JIT compilation.
...
| :--- | :--- | :--- | :--- |
| **1. Sampling Path** | CUDA only supports greedy ($T=0$) or pure temperature via Gumbel-max ([`src/cuda_resident.rs:13228`](file://./src/cuda_resident.rs#L13228)). | Setting `top_p` or `top_k` forces a full D2H logits transfer (512 KB/token for Llama 3.2 128k vocab) and CPU sort, causing a 2–5ms stall per token. | **GPU-Native Fused Top-K / Top-P (Nucleus) / Min-P Sampling Kernel** executing 100% on GPU. |
| **2. Prefill Pipeline** | Dynamic Chunked Flash Prefill landed ([PR #670](https://github.com/timtoole02/Camelid/pull/670)), but [`src/inference.rs:3323`](file://./src/inference.rs#L3323) runs synchronous D2H KV dump. | Synchronously copying KV cache back to CPU on every prefill is redundant because lazy recovery ([`recover_cpu_kv_from_cuda_resident`](file://./src/inference.rs#L3419)) already handles CPU fallbacks. | **Eliminate eager D2H KV readback** on GPU prefill to cut TTFT latency. |
| **3. KV Cache Footprint** | Stored strictly in FP16 (`u16`) across all layers ([`src/cuda_resident.rs:9584`](file://./src/cuda_resident.rs#L9584)). | Long context (8k–32k tokens) consumes gigabytes of VRAM and saturates memory bandwidth in decode attention. | **Quantized KV Cache for CUDA (FP8 E4M3/E5M2 & Q8_0)**: Halves KV VRAM and doubles decode attention memory bandwidth. |
| **4. Speculative Decoding** | Greedy-only ($T=0$) verification in [`src/inference/speculative.rs`](file://./src/inference/speculative.rs). | Cannot run speculative decoding on conversational or creative prompts where $T > 0$ and $top\_p < 1.0$. | **Lossless Speculative Rejection Sampling ($T > 0$)**: Implement Leviathan et al. probability ratio acceptance criterion. |
| **5. Continuous Batching** | CPU round-robin scheduler in [`src/api/continuous_batch.rs`](file://./src/api/continuous_batch.rs) serializes GPU decode one-by-one. | Concurrent API requests serialize at the global GPU engine lock, wasting GPU compute capability. | **Batched Decode GEMV on CUDA ($M=2..16$)**: Amortize weight memory reads across multiple active request slots. |
...
- **Location**: [`src/cuda_resident.rs`](file://./src/cuda_resident.rs) & [`src/inference.rs`](file://./src/inference.rs)
...
- **Location**: [`src/inference.rs:3323-3331`](file://./src/inference.rs#L3323-L3331)
...
- **Location**: [`src/cuda_resident.rs:9584-9587`](file://./src/cuda_resident.rs#L9584-L9587) & [`src/tensor/kv_quant.rs`](file://./src/tensor/kv_quant.rs)
...
- **Location**: [`src/inference/speculative.rs`](file://./src/inference/speculative.rs)
...
- **Location**: [`src/api/continuous_batch.rs`](file://./src/api/continuous_batch.rs) & [`src/cuda_resident.rs`](file://./src/cuda_resident.rs)
...
- **Location**: [`src/cuda_resident.rs`](file://./src/cuda_resident.rs) & [`src/tensor/mod.rs`](file://./src/tensor/mod.rs)
...
- **Optimization #1**: GPU-Native Fused Gumbel / Min-P Sampling Kernel ([`src/cuda_resident.rs`](file://./src/cuda_resident.rs))
...
- **Optimization #2**: Lazy Host KV Materialization on GPU Prefill ([`src/inference.rs`](file://./src/inference.rs))
...
- **Optimization #3**: Quantized KV Cache for CUDA Resident Engine ([`src/cuda_resident.rs`](file://./src/cuda_resident.rs), [`src/inference.rs`](file://./src/inference.rs))
...
- **Optimization #4**: Leviathan et al. (2023) Lossless Speculative Rejection Sampling ([`src/inference/speculative.rs`](file://./src/inference/speculative.rs))
