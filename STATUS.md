...
- Runtime/API alignment is now green for the exact Llama 3.2 1B/3B second bounded 1024-context row+box: `/api/capabilities` exposes `bounded_context_window=1024`, `bounded_context_1024_pack=validated_second_pack`, `bounded_context_1024_pack_id=llama3-context-1024-smoke-v1`, and `bounded_context_1024_window=1024` for those two rows only; frontend API/Models surfaces display the 1024-context track without lending it to TinyLlama or 8B.
