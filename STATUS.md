...
| Llama 3.2 1B Instruct Q8_0 | Exact-row verified support moved beyond a demo: API, WebUI, compact/broader parity, bounded template-shapes, unique-chat RSS/perf, and post-fix 2048 evidence all agree. | 512, 1024, 2048, and 4096 bounded packs are green; 2048 is green only after the RoPE frequency-factor fix, and 4096 is green on source/runtime head `470388f8165b` for the compact-template recall pack. | Broader/full support still needs model-native/larger context, arbitrary/Jinja template coverage, production throughput, portability, and durable full-support normalization. |
