...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end short-context generation parity artifacts: compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack all agree for the exact tracked Q8_0 GGUF. The 8B 1024 and 2048 bounded context boxes remain red until fresh PASS artifacts and docs/API/frontend alignment promote those buckets.
...
- 8B 1024/2048 remain red in the support ledger until fresh PASS artifacts and docs/API/frontend alignment promote those buckets; older 1024/2048 attempts stay review history rather than current support truth.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows, while the exact Llama 3 8B row remains promoted only through the checked bounded 512-context pack; frontend surfaces must display that boundary.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, and both exact Llama 3.2 rows have separate checked 2048-context pack passes. The exact 8B 1024/2048 buckets remain red until fresh PASS artifacts and docs/API/frontend alignment promote those buckets.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and does **not by itself** promote context, 8B long-context support, or production throughput. The 8B 1024/2048 buckets remain red until fresh PASS artifacts and synchronized docs/API/frontend promotion.
...
- The current 8B context support truth for this exact row stops at the checked 512-context pack; 1024/2048 remain red until fresh PASS artifacts and docs/API/frontend alignment promote those buckets.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the checked 512 bounded context; 1024/2048, neighboring Llama rows, other quantizations, larger/model-native contexts, and broader chat-template behavior remain outside the claim.
