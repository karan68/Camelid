...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end short-context generation parity artifacts: compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack all agree for the exact tracked Q8_0 GGUF. The 8B 1024 and 2048 bounded context boxes are promoted only as exact-row pack claims by `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260506T231242Z-head-7263fb206e8d/manifest.json`.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260506T231242Z-head-7263fb206e8d/manifest.json` is the current clean-main Llama 3 8B 1024/2048 bounded-pack support citation: 1024 matched 881 prompt tokens plus generated IDs `[34,2735,35,12,4278]` / `CMLD-102`; 2048 matched 1910 prompt tokens plus generated IDs `[34,2735,35,12,7854]` / `CMLD-204`; the claim is exact-row bounded-pack only.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows and for the exact Llama 3 8B row as bounded pack claims only; frontend surfaces display that boundary.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, and both exact Llama 3.2 rows have separate checked 2048-context pack passes. The exact 8B 1024/2048 buckets are now bounded exact-row pack claims only after the clean current-main rerun.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The later 8B 1024/2048 bounded-pack support moved only after fresh PASS artifacts and synchronized docs/API/frontend promotion.
...
- The current 1024/2048 support truth for this exact 8B row is `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260506T231242Z-head-7263fb206e8d/manifest.json`; older 1024/2048 artifacts stay in review history.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through checked 512/1024/2048 bounded contexts; neighboring Llama rows, other quantizations, larger/model-native contexts, and broader chat-template behavior remain outside the claim.
