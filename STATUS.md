...
- The attempted third bounded 2048-context pack is not green for this exact 8B row: llama.cpp accepted the 1910-token bounded prompt and generated `CMLD-204`, but Camelid `/v1/chat/completions` did not complete within the 900-second parity harness timeout, so no parity report was produced.
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope; neighboring Llama rows, other quantizations, the blocked 2048-context pack, larger contexts, and broader chat-template behavior remain outside the claim.
