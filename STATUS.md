...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json` plus `SHA256SUMS` is source-head 8B 1024/2048 bounded-pack PASS evidence for source head `9f8588bb4a4e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters. After later source/API changes, it is historical evidence only and does not promote 8B 1024/2048 on current `main` without a fresh canonical PASS.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The `9f8588bb4a4e` 8B 1024/2048 PASS bundle remains source-head evidence and does not cover later source/API changes.
...
- The current 8B context support truth for this exact row includes the checked 512 bounded pack only. The 1024/2048 pass at `9f8588bb4a4e` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json`, but it is source-head evidence after later source/API changes; the earlier `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence too.
...
- the 1024/2048 bounded-context pack passed at source head `9f8588bb4a4e` with prompt-token, generated-token, and generated-text parity plus structured RSS/Q8 file-read counters; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/`, but later source/API changes make it historical until rerun
