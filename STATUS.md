...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. Mistral 7B Instruct v0.3 Q8_0 now has exact-row smoke support with checked bounded 512/1024/2048 prompt-pack evidence and API completion/chat smoke. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. Llama 3.2 1B/3B/8B retain bounded 512/1024/2048-context packs where row-specific PASS bundles are cited. The fresh 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T202751Z-head-86ad5390d265/manifest.json` passed at runtime/API/frontend head `86ad5390d265` with prompt-token, generated-token, and generated-text parity. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama/Mistral-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
- **Supported-row hardening:** keep TinyLlama as the current full gate, keep Llama 3.2 1B/3B, Llama 3 8B, and Mistral 7B Instruct v0.3 labeled as exact-row verified support within validated bounds until they meet the harder normalized bar, and treat CI reliability as a release blocker rather than a best-effort signal.
- **Active next-model bring-up set:** Camelid has promoted the first Mistral exact row to bounded smoke support and is now publicly working on three follow-on exact candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — bounded exact-row smoke support is now green through checked 512/1024/2048 packs; 50-token parity, broader prompts, arbitrary templates, throughput, portability, and broader/full support remain blocked.
...
Public readiness language is locked to exact-row evidence: Mistral may say “supported only for exact-row smoke plus checked bounded 512/1024/2048 packs,” while Mixtral, Qwen, and Gemma may say “planned exact-row candidate, not supported yet.” Any next-family promotion requires row-specific source/SHA/license, tokenizer/template references, parity, bounded load/readiness, API/WebUI, RSS/timing, scrubbed manifest, and checksum evidence; Mixtral also needs expert-routing proof or typed unsupported behavior.
...
Bottom line for reviewers: Camelid has the original TinyLlama verified support gate plus four larger exact Q8_0 rows with verified support within validated bounds: Llama 3.2 1B, Llama 3.2 3B, Llama 3 8B, and Mistral 7B Instruct v0.3. That is a real end-to-end support expansion, but it is not a broad Llama- or Mistral-family claim.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T202751Z-head-86ad5390d265/manifest.json` plus `SHA256SUMS` is the fresh exact Llama 3 8B Q8_0 bounded 1024/2048 PASS bundle for runtime/API/frontend head `86ad5390d265`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`. It promotes only those exact bounded packs, not model-native/larger context, arbitrary templates, neighboring rows, broad 8B/Llama-family support, portability, or production throughput.
- `qa/evidence-bundles/mistral-7b-v0.3-q8-context-512-1024-2048-current-head-20260508T195143Z-head-e50de923bd62/manifest.json` plus `SHA256SUMS` is the exact Mistral 7B Instruct v0.3 Q8_0 bounded 512/1024/2048 PASS bundle for source head `e50de923bd62`; all three packs matched prompt tokens, generated token IDs, and generated text (` The repeat marker is "`) and API completion/chat smoke passed. It promotes only exact-row smoke plus checked bounded packs, not Mistral-family, neighboring variants, 50-token parity, arbitrary-template, model-native/larger-context, throughput, portability, or full support.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B/8B rows where row-specific PASS artifacts exist; the 8B 1024/2048 promotion is limited to the fresh bounded-pack evidence for runtime/API/frontend head `86ad5390d265`.
...
- Bounded 512-context evidence now exists for the original four Llama-family exact Q8_0 rows and for the new exact Mistral 7B Instruct v0.3 Q8_0 row. This is checked bounded-pack evidence only, not a model-native context or portability claim. The exact Llama 3.2 1B, Llama 3.2 3B, Llama 3 8B, and Mistral rows have checked support through 1024-context and 2048-context packs where listed.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The fresh `86ad5390d265` 8B 1024/2048 PASS bundle is the row-specific bounded-pack evidence for that exact 8B promotion.
...
- The current 8B context support truth for this exact row includes the checked 512/1024/2048 bounded packs where cited. The fresh 1024/2048 pass for runtime/API/frontend head `86ad5390d265` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T202751Z-head-86ad5390d265/manifest.json`; the earlier `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence.
...
- the 1024/2048 bounded-context pack passed at runtime/API/frontend head `86ad5390d265` with prompt-token, generated-token, and generated-text parity; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T202751Z-head-86ad5390d265/`; older 1024/2048 bundles remain historical for their source heads
...
6. Harden the first Mistral exact-row support lane: preserve `Mistral-7B-Instruct-v0.3.Q8_0.gguf` exact-row smoke plus checked 512/1024/2048 bounded-pack support, keep broader Mistral-family support fail-closed, and add 50-token parity, broader prompts, arbitrary-template evidence, throughput, portability, and repeated current-head API/WebUI/RSS bundles before any wider wording.
...
### Mistral exact-row support note

Mistral work has its first bounded exact-row promotion. Camelid now supports `Mistral-7B-Instruct-v0.3.Q8_0.gguf` only for exact-row smoke plus checked bounded 512/1024/2048 packs. The promotion is anchored by the SHA-pinned first-token bundle and the current bounded context bundle at `qa/evidence-bundles/mistral-7b-v0.3-q8-context-512-1024-2048-current-head-20260508T195143Z-head-e50de923bd62/manifest.json`: prompt-token parity, generated-token parity, generated-text parity, `/v1/health`, `/v1/completions`, and `/v1/chat/completions` all passed for the exact row.

The boundary stays strict: this is not Mistral-family support, neighboring-version support, 50-token parity, arbitrary/Jinja template support, production throughput, portability, model-native/larger-context support, or full support. The next Mistral status-changing work should add broader prompt packs, 50-token parity, repeated current-head API/WebUI/RSS evidence, arbitrary-template coverage, production-throughput measurements, portability evidence, and normalized full-support bundles before any broader wording appears.
