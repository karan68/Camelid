...
- `qa/evidence-bundles/supported-row-template-throughput-blocker-proof-20260513T2049Z-head-994569dbf995/manifest.json` records the current supported-row closeout proof for the arbitrary/Jinja-template and production-throughput caveats: new runtime/API guardrails passed and show Camelid does not execute arbitrary Jinja templates as a supported renderer, does not expose production-throughput evidence for current supported rows, and has a shortest path for clearing both blockers with row-specific template parity plus production-throughput harness evidence.
