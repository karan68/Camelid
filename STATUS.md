...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. On current `main` at `9f8588bb4a4e`, Llama 3.2 1B/3B and Llama 3 8B now have checked bounded 512/1024/2048-context packs for their exact Q8_0 rows. The current 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json` passed with prompt-token, generated-token, generated-text, RSS, and Q8 file-read counters for `CMLD-102`/`CMLD-204`. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity/context envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and checked 512/1024/2048-context packs for the exact tracked Q8_0 GGUF. The current 1024/2048 bounded prompt-pack pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json` is current-head evidence for `9f8588bb4a4e`; earlier `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` passes remain historical source-head evidence only.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json` plus `SHA256SUMS` is current-head 8B 1024/2048 bounded-pack PASS evidence for source head `9f8588bb4a4e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, records elapsed wall clock `4:06.04` / `8:45.43`, max RSS `17383048` / `17571772` KiB, and structured Q8 file-read counters. This closes only exact-row bounded 1024/2048 context parity for Llama 3 8B Instruct Q8_0.
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` plus `SHA256SUMS` is historical source-head 8B 1024/2048 bounded-pack PASS evidence for source head `844e8d4709d1`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters; it remains historical evidence only after later source/runtime changes.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B and Llama 3 8B rows with row-specific PASS artifacts, while keeping broader/full support and model-native/larger context beyond checked packs unpromoted.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The later `9f8588bb4a4e` 8B 1024/2048 PASS bundle is the current-head long-context evidence for that exact 8B row; earlier `844e8d4709d1` evidence remains historical.
...
- The current 8B context support truth for this exact row includes checked 512/1024/2048 bounded packs. The current 1024/2048 pass at `9f8588bb4a4e` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json`; earlier `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence only.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` (historical source-head predecessor)
...
- the current 1024/2048 bounded-context pack passed at source head `9f8588bb4a4e` with prompt-token, generated-token, and generated-text parity plus structured RSS/Q8 file-read counters; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/`, while older `844e8d4709d1`/`e203d3cf3ea5` source-head passes remain historical
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including the checked bounded 512/1024/2048 context packs on current `main`. It is not broad Llama-family, model-native/larger-context beyond checked packs, production-throughput, portability, arbitrary-template, or neighboring-row support.
...
4. Preserve the Llama 3 8B exact-row smoke plus checked 512/1024/2048 promotion in docs, API, frontend readiness, and regression evidence without lending it to model-native/larger context, arbitrary templates, production throughput, portability, or neighboring rows.
