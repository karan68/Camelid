...
- Runtime/API alignment is now explicit for the exact Llama 3.2 1B/3B context row+boxes: `/api/capabilities` exposes the 1024 track as `validated_second_pack` for both exact rows, exposes 1B 2048 as `blocked_first_token_divergence`, exposes 3B 2048 as `validated_third_pack`, and keeps 8B 2048 as `ready_to_run_not_promoted`; frontend API/Models surfaces display those row-specific tracks without lending them across rows.
