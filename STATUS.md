...
- **Active next-model bring-up set:** Camelid is publicly working on exact next-family rows, with Mixtral/Qwen/Gemma as follow-on candidates.
  ...
  - **Gemma 4 lane (branch `feature/gemma4-performance-support`):** `gemma-4-E2B-it-Q8_0.gguf` and `gemma-4-E4B-it-Q8_0.gguf` are supported exact-row text-token-generation smoke on the from-scratch `gemma4` engine — five-prompt greedy parity (prompt ids, generated ids, text) against the pinned llama.cpp `5d56eff` oracles (`qa/gemma4/oracle/`), API/streaming chat smoke behind `CAMELID_GEMMA4_SERVE`, the corrected `<|turn>`/`<turn|>` marker template with thinking-channel suppression, and typed fail-closed multimodal rejection. Distributed layer sharding (`gemma4-master`/`gemma4-worker`, versioned handshake + checksummed packets) is parity-locked by `tests/gemma4_distributed_parity.rs`. `gemma-4-12b-it-Q8_0.gguf` is active validation (single-node CPU memory-bound on 16 GB; two-node sharding is its lane); 26B A4B / 31B / MTP rows fail closed with named blockers.
