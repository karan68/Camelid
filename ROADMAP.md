...
- **Next-family posture:** Mistral 7B Instruct v0.3 Q8_0 and LFM2.5 2.6B Q8_0 are supported exact-row smoke lanes, with LFM2 scoped to Windows CPU runnable chat and one Apple M4 macOS 26.5 arm64 resident-Metal host through its checked 512-token bucket. Mixtral 8x7B Instruct v0.1 Q8_0 is blocked partial runtime evidence only, and Qwen/Gemma 2 remain planned exact-row candidates; Gemma 4 E2B-It/E4B-It Q8_0 are promoted exact-row text-generation smoke lanes with 12B-it in active validation behind the two-node sharding lane.
...
Near-term thesis: protect the trusted TinyLlama gate, the exact Llama 3.2 1B/3B and Llama 3 8B bounded rows, keep the Mistral 7B Instruct v0.3 Q8_0 and the Windows CPU/runnable plus receipted Apple M4 resident-Metal LFM2.5 2.6B Q8_0 exact-row smoke language synchronized with their promotion evidence, keep Mixtral fail-closed until its blockers are fixed, and advance Ubuntu x86 performance only through default-off, measured, parity-preserving slices.
...
- **Cite committed evidence anchors first.** The public bundle manifest/checksums, perf/portability envelope, reopened-lane API + frontend smoke manifest, 1B bounded 1024/2048/4096/8192 context bundles, the 3B anchored 512-8192 ladder bundle plus historical canonical API/WebUI bundle, the current-head 8B 1024/2048 bundle, Mistral validation bundles, the LFM2 Phase 1 Windows promotion bundle and exact-row M4 Metal handoff bundle, Mixtral blocker bundles, and current-head per-row manifests are the roadmap-facing evidence layer; raw `target/` artifacts are drill-down only.
...
- LFM2.5-2.6B `Q8_0` completed all eight Phase 1 gates and is promoted as `supported_exact_row_smoke` on the Windows x86_64 CPU runnable-chat lane. The Windows bundle fixes the exact file identity, 96/96 short generated IDs, an exact 512-prompt-token native receipt with 8/8 generated IDs/text, API/SSE behavior, and Models-page readiness. The completed Mac handoff independently repeats those gates on an Apple M4 host running macOS 26.5 arm64, asserts the `metal_resident_lfm2_runtime` prefill/decode plan, and passes API/Models-page/WebUI non-streaming plus 128-ceiling SSE smoke. No other Apple hardware or broad portability inherits that result.
- Mistral Nemo Instruct 2407 Q4_K_M, Qwen3 14B Q4_K_M, and DeepSeek R1 0528 Qwen3 8B Q4_K_M are exact downloadable catalog rows under active validation or hold, not support. Their bounded bring-up evidence is row-specific: Nemo and DeepSeek have recorded CPU/Metal divergence, while Qwen3 14B lacks an external oracle and chat/API gate.

Near-term objective: preserve every supported exact row across all support surfaces without broadening any of them, protect the narrow LFM2.5 Windows CPU/runnable and single-host Apple M4 resident-Metal receipts while qualifying context above 512 and any further portability independently, close the remaining active-validation lanes (Mixtral, Mistral Nemo Q4_K_M, Qwen3 14B Q4_K_M, DeepSeek R1 0528 Qwen3 8B Q4_K_M, and the two distributed Gemma 4 rows) or keep them visibly unpromoted, clear the Phi-3 HOLD blockers before any Phi wording, and keep performance and GPU claims scoped to the exact row, host, and device they were measured on.
...
- Protect the Mistral 7B Instruct v0.3 Q8_0, Windows CPU/runnable plus receipted Apple M4 resident-Metal LFM2.5 2.6B Q8_0, five dense Qwen3, three Ornith `qwen35`, Gemma 3 1B, Gemma 4 E2B/E4B, seven PrismML Bonsai, and four certified Llama requant rows across docs, `/api/capabilities`, and frontend readiness.
...
- Keep Mistral Nemo Q4_K_M and DeepSeek R1 0528 Qwen3 8B Q4_K_M on support hold until their recorded cross-backend divergences and row-specific promotion gaps close; keep Qwen3 14B Q4_K_M active-validation until an external oracle and chat/API evidence exist.
- Keep the two distributed Gemma 4 rows visibly active-validation, not supported; do not let LFM2's one promoted artifact and two receipted execution lanes widen their status or imply family-wide LFM2 support.
...
- Close the remaining active-validation set as exact-row lanes, never as family-wide claims: Mixtral 8x7B Instruct v0.1 Q8_0, Mistral Nemo Instruct 2407 Q4_K_M, Qwen3 14B Q4_K_M, DeepSeek R1 0528 Qwen3 8B Q4_K_M, and the two distributed Gemma 4 rows.
- Qualify LFM2 bounded context above 512 tokens and any broader sizes, quants, sampling, tool behavior, CUDA lane, or additional Apple hardware independently; none inherits the exact row's Windows CPU/runnable or single-host M4 resident-Metal smoke result.
...
| LFM2.5 2.6B Q8_0 exact-row CPU/runnable + M4 resident-Metal smoke | Complete / supported exact-row smoke | `qa/evidence-bundles/lfm2-2.6b-q8-phase1-promotion-20260810/` records all eight Phase 1 gates passing on Windows x86_64 CPU. `qa/evidence-bundles/lfm2-2.6b-q8-macos-metal-20260810-head-d31e5cb0/` closes the handoff on Apple M4 / macOS 26.5 / arm64 with 96/96 short generated IDs, exact 512-plus-8 parity, asserted resident-Metal health paths, and API/Models-page/WebUI non-streaming plus 128-ceiling SSE smoke. Context above 512, adjacent rows/quants, tools, broader sampling, CUDA, other Apple hardware, broad portability, and throughput remain gated. |
...
- LFM2.5 2.6B Q8_0 is supported exact-row smoke on the Windows x86_64 CPU runnable-chat lane and, independently, the resident-Metal lane on one Apple M4 host running macOS 26.5 arm64, through exactly 512 rendered prompt tokens. Its Phase 1 Windows bundle and completed Mac handoff align immutable identity, short and receipt parity, API/SSE, `/api/capabilities`, Models-page readiness, and the M4 resident-Metal health plan; raw `/v1/completions` and tools remain typed fail-closed. Context above 512, adjacent artifacts, broader sampling, CUDA, other Apple hardware, broad portability, and throughput require separate evidence.
...
- LFM2.5 2.6B Q8_0 has checked exact 512-rendered-token chat receipts on the Windows CPU runnable lane and the resident-Metal lane on one Apple M4 / macOS 26.5 / arm64 host; no larger bucket, other Apple hardware, or broad portability claim is promoted.
