...
Near-term thesis: protect the trusted TinyLlama gate, the exact Llama 3.2 1B/3B and Llama 3 8B bounded rows, keep the Mistral 7B Instruct v0.3 Q8_0 and Windows CPU/runnable LFM2.5 2.6B Q8_0 exact-row smoke language synchronized with their promotion evidence, keep Mixtral fail-closed until its blockers are fixed, and advance Ubuntu x86 performance only through default-off, measured, parity-preserving slices.
...
- **Cite committed evidence anchors first.** The public bundle manifest/checksums, perf/portability envelope, reopened-lane API + frontend smoke manifest, 1B bounded 1024/2048/4096/8192 context bundles, the 3B anchored 512-8192 ladder bundle plus historical canonical API/WebUI bundle, the current-head 8B 1024/2048 bundle, Mistral validation bundles, the LFM2 Phase 1 promotion bundle, Mixtral blocker bundles, and current-head per-row manifests are the roadmap-facing evidence layer; raw `target/` artifacts are drill-down only.
...
- LFM2.5-2.6B `Q8_0` completed all eight Phase 1 gates and is promoted as `supported_exact_row_smoke` on the Windows x86_64 CPU runnable-chat lane. The committed bundle fixes the exact file identity, 96/96 short generated IDs, an exact 512-prompt-token native receipt with 8/8 generated IDs/text, API/SSE behavior, and Models-page readiness. Apple-Silicon resident-Metal parity remains a separate handoff, not inherited support.

Near-term objective: preserve every supported exact row across all support surfaces without broadening any of them, protect the narrow LFM2.5 Windows CPU/runnable promotion while handing its exact row to Apple-Silicon Metal validation, close the remaining active-validation lanes (Mixtral and the two distributed Gemma 4 rows) or keep them visibly unpromoted, clear the Phi-3 HOLD blockers before any Phi wording, and keep performance and GPU claims scoped to the exact row, host, and device they were measured on.
...
- Protect the Mistral 7B Instruct v0.3 Q8_0, Windows CPU/runnable LFM2.5 2.6B Q8_0, five dense Qwen3, three Ornith `qwen35`, Gemma 3 1B, Gemma 4 E2B/E4B, seven PrismML Bonsai, and four certified Llama requant rows across docs, `/api/capabilities`, and frontend readiness.
...
- Keep the two distributed Gemma 4 rows visibly active-validation, not supported; do not let LFM2's one promoted CPU/runnable row widen their status or imply family-wide LFM2 support.
...
- Execute the LFM2 bundle's Mac handoff for the exact LFM2.5 2.6B Q8_0 row, then qualify bounded context above 512 tokens and any broader sizes, quants, sampling, or tool behavior independently; none inherits the Windows CPU/runnable smoke result.
...
| LFM2.5 2.6B Q8_0 exact-row CPU/runnable smoke | Complete / supported exact-row smoke | `qa/evidence-bundles/lfm2-2.6b-q8-phase1-promotion-20260810/` records all eight Phase 1 gates passing on Windows x86_64 CPU: exact identity, 96/96 short generated IDs, exact 512-token receipt parity, runnable chat/SSE, and Models-page readiness. Metal, context above 512, adjacent rows/quants, tools, broader sampling, and throughput remain gated. |
...
- LFM2.5 2.6B Q8_0 is supported exact-row smoke on the Windows x86_64 CPU runnable-chat lane through exactly 512 rendered prompt tokens. Its Phase 1 bundle aligns immutable identity, tokenizer/template, short and receipt parity, API/SSE, `/api/capabilities`, and Models-page readiness; raw `/v1/completions` and tools remain typed fail-closed. Apple-Silicon Metal, context above 512, adjacent artifacts, broader sampling, and throughput require separate evidence.
...
- LFM2.5 2.6B Q8_0 has one checked exact 512-rendered-token chat receipt on the Windows CPU runnable lane; no larger bucket or Metal context claim is promoted.
